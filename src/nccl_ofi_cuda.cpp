/*
 * Copyright (c) 2018-2023 Amazon.com, Inc. or its
...
#define RESOLVE_CUDA_FUNCTION(function)                                                         \
  do {                                                                                          \
    ...
    cudaError_t err = cudaGetDriverEntryPoint(QUOTE(function), (void **)&pfn_##function,        \
...
bool nccl_net_ofi_cuda_have_gdr_support_attr(void)
{
  ...
  result = pfn_cuDeviceGetAttribute(
      &supported, CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_SUPPORTED, dev);
  if (result != CUDA_SUCCESS || !((bool)supported)) {
    return false;
  }

  result = pfn_cuDeviceGetAttribute(
      &supported, CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_FLUSH_WRITES_OPTIONS, dev);
  return result == CUDA_SUCCESS &&
         ((supported & CU_FLUSH_GPU_DIRECT_RDMA_WRITES_OPTION_HOST) != 0);
#else
  return false;
#endif
}
...
bool nccl_net_ofi_cuda_have_dma_buf_attr(void)
{
  ...
  int supported;
  result = pfn_cuDeviceGetAttribute(
      &supported, CU_DEVICE_ATTRIBUTE_DMA_BUF_SUPPORTED, dev);
  if (result != CUDA_SUCCESS) {
    return false;
  }
